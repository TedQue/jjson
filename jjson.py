...
# 性能问题:
# 0. 保证只遍历一次输入字符串
# 1. 输入字符串可能非常大,参数以 [s, len(s)] 列表形式传递上下文
#    (大约没什么用,与直接用 str 作为参数一样,但是采用"上下文"打包传送便于以后改进)
# 2. 递归是否会导致堆栈溢出?
...
class jjson_decode_error(Exception):
    ...
    def __str__(self):
        return f"at posion {self._pos} "
        f"(row {self._row + 1}, col {self._col + 1})"
...
def read(s, p):
    ...
    if check(s, p):
        ...
        if c == '[':
            ...
            return read_dict(s, p)
        elif c in string_lead_char:
            return read_string(s, p)
        elif c in number_lead_char:
            return read_number(s, p)
        elif c in id_lead_char:
            return read_id(s, p)
    raise jjson_decode_error(s, p)
